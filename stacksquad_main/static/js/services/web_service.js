...
document.querySelectorAll('.section-label,.section-title,.section-sub,.inc-card,.process-step,.tool-chip,.faq-item,.cta-box,.stat-item').forEach(function (el, i) {
    var parent = el.closest('.included-grid,.tools-flex,.faq-list,.stats-row');
    if (parent) { var idx = Array.from(parent.children).indexOf(el); el.style.transitionDelay = (idx * 70) + 'ms,' + (idx * 70) + 'ms'; }
    obs.observe(el);
});
